...
(function () {
  ...
  function drawQuestion(cat, forceNewAnimation = true) {
    ...
    if (forceNewAnimation && cardEl) {
      triggerFade(cardEl, selectedQuestion, cat);
    } else {
      if (categoryRibbonEl) {
        categoryRibbonEl.textContent = cat === 'this_year' ? '🌿 올해를 돌아보는 질문' : '✨ 새해를 맞이하는 질문';
      }
  ...
  function triggerFade(element, newQuestionText, cat) {
    if (isAnimating) return;
    isAnimating = true;
    ...
    // 1. Fade out question text
    element.classList.remove('fading-in');
    element.classList.add('fading-out');

    // 2. After fade out (250ms), change text and ribbon, then fade in
    setTimeout(() => {
      if (questionTextEl) {
